...
Esse guia de estilo é um produto da nossa equipe de desenvolvimento Android. Reflete regras de programação da linguagem
Java que observamos ser eficientes no desenvolvimento de nossos projetos de aplicativos.
...
Alguns dos padrões que aqui podem ser observados podem opor os padrões seguidos pela comunidade Android, porém ressaltamos que testamos e utilizamos essas práticas em nossa equipe e as mesmas se mostraram serem mais eficientes até o presente momento.
...
Dito isso, esse é um documento que será atualizado constantemente, a medida que nossa equipe e a linguagem Java evoluam, nossas práticas irão se adaptar acompanhando essas mudanças. Esse guia foi atualizado pela última vez em _ de _ de 2017.
...
* **1.2.** Utilizar o comando de indentação do Android Studio, e não alterar nada depois. O comando pode ser feito com o atalho ***control + alt + l*** no Windows, ***control + shift + alt + l*** no Linux e ***option + command + l*** no Mac.
...
* **1.4.** Todas as funçes devem ser separadas uma da outra por uma linha vazia.
```java
// Certo
class UseClass{
    // ...
...
* **1.5.** Ao usar virgula (' , ') deve ser adicionado um espaço depois e não conter espaço algum antes.
...
* **1.6.** Inserir um epaço antes e depois de operadores binários ('+ , -, /, e etc') e conter um espaço antes de abrir parenteses e um depois de fechar.
```java
//Certo
...
* **1.7** Nunca use uma mais de uma linha em branco consecutiva. Use uma linha de separação em branco para separar áreas dos códigos com contextos diferentes. NUNCA use uma nova linha em branco depois de abrir uma chave.
```java
//Certo
...
* **2.3.** Divida a classe em regiões. Geralmente os arquivos das classes têm as seguintes regiões:
        ...
        * Quais quer outros métodos (publics, protected e private nessa ordem) -> Instance Methods
...
* **3.4.** Identificadores nos nomes devem ser ordenados do mais geral para o mais específico. Isso mantém os identificadores na mesma ordem da convenço de nomeação dos pacotes e das classes.
```java
//Certo
...
* **3.5.** Para os layouts, utilizar o padrão *nomedaclasse_tipo_funcao* para os *ids* de todas as *views*. Utilizar os seguintes prefixos:
        ...
        * Caso no seja nenhum dos itens acima, usar algo de descreve o uso de uma maneira similar às descritas acima.
...
**3.1.2** Utilizar ao máximo as funções já existentes do Java. Não há necessidade de se escrever código de ordenação, por exemplo.
